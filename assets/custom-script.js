window.lucellia = window.lucellia || {};
...
(function(){
  ...
  theme.Drawers = (function() {
    function Drawers(id, name) {
      ...
      this.nodes = {
        page: document.querySelector('#MainContentArea')
      };
  ...
  // If no matching group was found, simply exit.
  if (!group) return;
  
  // Escape the value for safe selector usage.
  var value = obj.value.replace(/([ #;&,.+*~\':"!^$[\]()=>|\/@])/g, '\\$1');

  if (this.type === 'dropdown') {
    // Get the option element.
    var optionEl = group.querySelector('option[value="'+ value +'"]');
    if (optionEl) {
      // Set disabled property based on sold out state.
      optionEl.disabled = obj.soldOut;
    }
  } else {
    // For button style inputs.
    ...
    // Enable variant option.
    input.classList.remove(classes.disabled);
    label.classList.remove(classes.disabled);
    // If sold out, add the disabled class.
    if (obj.soldOut) {
...
,
  ...
  theme.videoModal = function() {
    ...
    var modal = new theme.Modals('VideoModal', 'custom-video-modal', {
      closeOffContentClick: true,
      bodyOpenClass: ['modal-open', 'custom-video-modal-open'],
      solid: true
    });
  ...
  class ToolTip extends HTMLElement {
    constructor() {
      super();
      this.el = this;
      this.inner = this.querySelector('[data-custom-tooltip-inner]');
      this.closeButton = this.querySelector('[data-custom-tooltip-close]');
      this.toolTipContent = this.querySelector('[data-custom-tooltip-content]');
      this.toolTipTitle = this.querySelector('[data-custom-tooltip-title]');
  
      this.triggers = document.querySelectorAll('[data-custom-tooltip-trigger]');
  ...
  customElements.define('custom-tooltip-trigger', ToolTipTrigger);
  ...
  class ImageElement extends HTMLElement {
    ...
    removeAnimations() {
      // Find the closest image-container and remove the animation (fix for streamline image-container shimmer)
      const imageWrap = this.closest('.image-container');
  ...
  theme.headerNav = (function() {
    ...
    function stickyHeaderInitialPosition(header) {
      const headerParent = header.closest('.shopify-section-group-custom-header');
      ...
      if (parentNextSibling && parentNextSibling.classList.contains('shopify-section-group-custom-header')) {
  ...
  theme.buildProductImage = function(product, imageSizes) {
    ...
    if (size === 'natural') {
      const template = document.getElementById("customNaturalImage");
      const clonedMarkup = template.content.cloneNode(true);
      const imageEl = clonedMarkup.querySelector('img');
      const imageWrapperEl = clonedMarkup.querySelector('.image-container');
      ...
      const template = document.getElementById("customFixedRatioImage");
      const clonedMarkup = template.content.cloneNode(true);
      const imageEl = clonedMarkup.querySelector('img');
  ...
  class AgeVerificationPopup extends HTMLElement {
    ...
    init() {
      ...
      theme.a11y.lockMobileScrolling(`#${this.id}`, document.querySelector('#MainContentArea'));
      ...
      if (this.exitButton) {
        this.exitButton.addEventListener('click', (e) => {
          ...
          theme.a11y.unlockMobileScrolling(`#${this.id}`, document.querySelector('#MainContentArea'));
        })
      }
    }
